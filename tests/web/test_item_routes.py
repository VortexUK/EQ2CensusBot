...
def items_db_path(tmp_path: Path) -> Generator[Path]:
    ...
    with patch("census.db.DB_PATH", db_file):
        with patch("web.routes.item.DB_PATH", db_file):
            yield db_file
...
async def test_search_with_filter_when_db_missing_returns_503(app):
    ...
    with patch("web.routes.item.DB_PATH", Path("/nonexistent/items.db")):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_item_detail_unknown_id_falls_back_to_census_and_returns_item(app):
    ...
    with patch("web.routes.item.shared_census_client", return_value=mock_ctx):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_spell_scroll_apprentice_is_not_craftable(app):
    ...
    with (
        patch("web.routes.item.DB_PATH", Path("/nonexistent/items.db")),
        patch("web.routes.item.RECIPES_DB_PATH", Path("/nonexistent/recipes.db")),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_spell_scroll_expert_returns_recipe_when_found(app, tmp_path):
    ...
    with (
        patch("web.routes.item.find_by_spell", return_value=[fake_recipe]),
        patch("web.routes.item.DB_PATH", Path("/nonexistent/items.db")),
        patch("web.routes.item.RECIPES_DB_PATH", fake_recipes_db),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_items_filters_returns_server_max_level(app):
    ...
    with patch("web.routes.item.current_server", return_value=fake_server):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
