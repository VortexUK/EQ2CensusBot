...
class TestGetUpgradeMaterials:
    async def test_spells_db_missing_returns_503(self, app):
        ...
        with (
            patch(
                "web.routes.character.upgrades._SPELLS_DB",
    ...
    async def test_recipes_db_missing_returns_503(self, app, tmp_path):
        ...
        with (
            patch("web.routes.character.upgrades._SPELLS_DB", new=fake_spells_db),
            patch(
                "web.routes.character.upgrades._RECIPES_DB",
    ...
    async def test_character_not_found_returns_404(self, app, tmp_path):
        ...
        fake_recipes.touch()
        with (
            patch("web.routes.character.upgrades._SPELLS_DB", new=fake_spells),
            patch("web.routes.character.upgrades._RECIPES_DB", new=fake_recipes),
            patch(
                "web.routes.character.upgrades.character_cache.get_stale",
                return_value=(None, False),
            ),
            patch("web.routes.character.upgrades.shared_census_client") as mock_ctx,
        ):
    ...
    async def test_no_sub_expert_spells_returns_zero_counts(self, app, tmp_path):
        ...
        with (
            ...
            patch(
                "web.routes.character.upgrades._spell_find_by_ids",
                return_value={101: expert_row},
            ),
            patch("web.routes.character.upgrades._load_spell_blocklist", return_value=set()),
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                r = await client.get("/api/character/Sihtric/upgrade-materials")
        assert r.status_code == 200
    ...
    async def test_happy_path_returns_sorted_ingredients(self, app, tmp_path):
        ...
        with (
            ...
            patch("web.routes.character.upgrades._load_spell_blocklist", return_value=set()),
            patch(
                "web.routes.character.upgrades._find_spell_recipes",
                return_value={"Divine Favor": fake_recipe},
            ),
            patch(
                "web.routes.character.upgrades._lookup_items_by_name",
                return_value={},
            ),
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
    ...
    async def test_two_recipes_sum_shared_ingredient(self, app, tmp_path):
        ...
        with (
            ...
            patch("web.routes.character.upgrades._spell_find_by_ids", return_value=rows),
            patch("web.routes.character.upgrades._load_spell_blocklist", return_value=set()),
            patch("web.routes.character.upgrades._find_spell_recipes", return_value=recipes),
            patch("web.routes.character.upgrades._lookup_items_by_name", return_value={}),
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                r = await client.get("/api/character/Sihtric/upgrade-materials")
        assert r.status_code == 200
...
class TestGetUpgradeRecipes:
    ...
    async def test_happy_path_returns_recipe_list(self, app, tmp_path):
        ...
        with (
            ...
            patch("web.routes.character.upgrades._spell_find_by_ids", return_value={101: adept_row}),
            patch("web.routes.character.upgrades._load_spell_blocklist", return_value=set()),
            patch(
                "web.routes.character.upgrades._find_spell_recipes",
                return_value={"Divine Favor": fake_recipe},
...
class TestLookupItemsByName:
    ...
    def test_exact_match_after_stripping_raw_prefix(self, tmp_path):
        ...
        from web.routes.character.upgrades import _lookup_items_by_name

        db_path = self._create_items_db(tmp_path)
        with patch("web.routes.character.upgrades._ITEMS_DB", new=db_path):
    ...
    def test_returns_empty_when_items_db_absent(self, tmp_path):
        ...
        with patch("web.routes.character.upgrades._ITEMS_DB", new=missing):
