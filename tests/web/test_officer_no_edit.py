...
async def test_officer_cannot_put_encounter_strategy(app):
    ...
    with (
        patch("web.auth_deps.require_user_session", return_value={"id": "officer-X", "username": "officer"}),
...
async def test_contributor_still_can_put_strategy(app):
    ...
    with (
        patch("web.auth_deps.require_user_session", return_value={"id": "contrib-1", "username": "contrib"}),
        patch("web.auth_deps.is_admin", return_value=False),
        patch("web.auth_deps.users_db.user_has_capability_via_db", new_callable=AsyncMock, return_value=True),
        patch("web.routes.raid_strategies.zones_db.find_by_name", return_value=_fake_zone()),
        patch("web.routes.raid_strategies._write_strategy_sync", return_value=fresh_row),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.put(
                "/api/zones/The Emerald Halls/encounters/1/strategy",
...
async def test_contributor_still_can_put_overview(app):
    ...
    with (
        ...
        patch("web.auth_deps.users_db.user_has_capability_via_db", new_callable=AsyncMock, return_value=True),
        patch("web.routes.raid_strategies.zones_db.find_by_name", return_value=_fake_zone()),
        patch("web.routes.raid_strategies._write_overview_sync", return_value=fresh_row),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.put(
                "/api/zones/The Emerald Halls/overview",
