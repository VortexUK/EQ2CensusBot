...
from web.routes.rankings import _cached_zones_data, invalidate_zones_cache
...
async def test_reorder_encounters_invalidates_cache(app, editor_override) -> None:
    ...
    with (
        ...
        patch("web.routes.zones_admin.zones_db.reorder_encounters"),
    ):
        _cached_zones_data()
        assert _cached_zones_data.cache_info().currsize == 1
...
async def test_edit_encounter_invalidates_cache(app, editor_override) -> None:
    ...
    with (
        ...
        patch(
            "web.routes.zones_admin.zones_db.update_encounter",
            return_value={
                "id": 5,
                "zone_id": 1,
...
async def test_delete_encounter_invalidates_cache(app, editor_override) -> None:
    ...
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch("web.routes.zones_admin.zones_db.delete_encounter", return_value=True),
    ):
        _cached_zones_data()
        assert _cached_zones_data.cache_info().currsize == 1
...
async def test_delete_encounter_no_invalidation_on_404(app, editor_override) -> None:
    ...
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch("web.routes.zones_admin.zones_db.delete_encounter", return_value=False),
    ):
        _cached_zones_data()
        assert _cached_zones_data.cache_info().currsize == 1
...
async def test_create_mob_invalidates_cache(app, editor_override) -> None:
    ...
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch(
            "web.routes.zones_admin.zones_db.add_mob",
...
async def test_edit_mob_invalidates_cache(app, editor_override) -> None:
    ...
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch(
            "web.routes.zones_admin.zones_db.update_mob",
...
async def test_promote_mob_invalidates_cache(app, editor_override) -> None:
    ...
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch(
            "web.routes.zones_admin.zones_db.promote_mob",
...
async def test_delete_mob_invalidates_cache(app, editor_override) -> None:
    ...
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch("web.routes.zones_admin.zones_db.delete_mob", return_value=True),
    ):
        _cached_zones_data()
        assert _cached_zones_data.cache_info().currsize == 1
...
async def test_delete_mob_no_invalidation_on_404(app, editor_override) -> None:
    ...
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch("web.routes.zones_admin.zones_db.delete_mob", return_value=False),
    ):
        _cached_zones_data()
        assert _cached_zones_data.cache_info().currsize == 1
