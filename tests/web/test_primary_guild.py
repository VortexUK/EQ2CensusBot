...
from web.lib.primary_guild import cached_primary_guild, get_primary_claim
...
class TestCachedPrimaryGuild:
    ...
    async def test_returns_character_and_guild_from_cache(self):
        ...
        with (
            patch("web.lib.primary_guild.get_active_claims", new=AsyncMock(return_value=claims_payload)),
            patch("web.lib.primary_guild.character_cache") as mock_cache,
        ):
    ...
    async def test_returns_none_both_when_no_primary_claim(self):
        with (
            patch("web.lib.primary_guild.get_active_claims", new=AsyncMock(return_value={"approved": []})),
        ):
            char_name, guild_name = await cached_primary_guild("user-1", "Varsoon")
