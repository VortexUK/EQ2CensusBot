...
class TestMeAdmin:
    async def test_admin_user_has_is_admin_true_and_forced_approved(self, app) -> None:
        ...
        with (
            patch("web.routes.auth._ADMIN_IDS", {admin_id}),
            patch(
            ...
            patch(
                "web.routes.auth.get_user_access_status",
    ...
    async def test_non_admin_user_has_is_admin_false(self, app) -> None:
        ...
        with (
            patch("web.routes.auth._ADMIN_IDS", set()),
            patch(
                "web.routes.auth.list_roles_for_user",
                AsyncMock(return_value=[]),
            ),
...
class TestLogoutEdgeCases:
    ...
    async def test_logout_with_session_audit_logs_user_id(self, app) -> None:
        ...
        with patch("web.routes.auth.audit_log", side_effect=_capture_audit):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                client.cookies.set("session", cookie)
