...
async def test_create_encounter_unknown_zone_404(app, editor_override):
    with patch("web.routes.zones_admin.zones_db.find_by_name", return_value=None):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.post(
...
async def test_reorder_encounters_400_on_bad_permutation(app, editor_override):
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch(
            "web.routes.zones_admin.zones_db.reorder_encounters",
...
async def test_delete_mob_422_on_invariant_violation(app, editor_override):
    with (
        patch("web.routes.zones_admin.zones_db.find_by_name", return_value={"id": 1, "name": "Z"}),
        patch(
            "web.routes.zones_admin.zones_db.delete_mob",
