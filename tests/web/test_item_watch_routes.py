...
class TestGetItemWatches:
    ...
    async def test_non_officer_returns_403(self, app):
        ...
        with patch(
            "web.routes.item_watch._officer_chars",
    ...
    async def test_officer_returns_watch_list(self, app):
        ...
        with (
            ...
            patch(
                "web.routes.item_watch.list_item_watches",
                new=AsyncMock(return_value=[watch]),
            ),
            patch(
                "web.routes.item_watch._check_all_watches",
                new=AsyncMock(),
            ),
        ):
            async with AsyncClient(
...
class TestAddItemWatch:
    ...
    async def test_character_not_in_roster_returns_404(self, app):
        ...
        with (
            ...
            patch(
                "web.routes.item_watch._roster_rank_map",
                new=AsyncMock(return_value={}),
            ),
        ):
            async with AsyncClient(
    ...
    async def test_item_not_found_returns_404(self, app):
        ...
        with (
            ...
            patch("census.db.find_by_name", new=AsyncMock(return_value=None)),
            patch("web.routes.item_watch.shared_census_client") as mock_ctx,
        ):
    ...
    async def test_duplicate_watch_returns_409(self, app):
        ...
        with (
            ...
            patch("census.db.find_by_name", new=AsyncMock(return_value=fake_item_raw)),
            patch(
                "web.routes.item_watch.get_active_claims",
                new=AsyncMock(return_value=[]),
            ),
            patch(
                "web.routes.item_watch.get_primary_claim",
                return_value=None,
            ),
            patch(
                "web.routes.item_watch.guild_cache.get_stale",
                return_value=(None, False),
            ),
            patch(
                "web.routes.item_watch.add_item_watch",
    ...
    async def test_happy_path_creates_watch_and_returns_201(self, app):
        ...
        with (
            ...
            patch(
                "web.routes.item_watch._check_watch",
                new=AsyncMock(),
            ),
        ):
            async with AsyncClient(
...
class TestDeleteItemWatch:
    ...
    async def test_missing_watch_returns_404(self, app):
        ...
        with (
            ...
            patch(
                "web.routes.item_watch.remove_item_watch",
