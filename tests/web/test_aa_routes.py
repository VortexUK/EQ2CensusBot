...
class TestGetAaConfig:
    ...
    async def test_missing_limits_file_returns_zero_defaults(self, app) -> None:
        ...
        with patch("web.routes.aa._LIMITS") as mock_limits:
    ...
    async def test_limits_file_present_returns_xpac_values(self, app, tmp_path) -> None:
        ...
        with (
            patch("web.routes.aa._LIMITS", limits_file),
            patch("web.routes.aa.current_server", return_value=mock_server),
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                r = await client.get("/api/aa/config")
...
class TestGetAaTree:
    ...
    async def test_missing_tree_returns_404(self, app) -> None:
        ...
        with patch("web.routes.aa._load_tree_for_response", return_value=None):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
    ...
    async def test_existing_tree_returns_tree_response(self, app, tmp_path) -> None:
        ...
        with patch("web.routes.aa._TREES_DIR", trees_dir):
            _load_tree_for_response.cache_clear()
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
class TestGetCharacterAas:
    ...
    async def test_hot_cache_hit_returns_immediately(self, app) -> None:
        ...
        with patch("web.routes.aa.current_world", return_value="Varsoon"):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
    ...
    async def test_census_store_hit_returns_data(self, app) -> None:
        ...
        with (
            patch("web.routes.aa.current_world", return_value="Varsoon"),
            patch("web.routes.aa.aa_cache.get_stale", return_value=(None, False)),
            patch("web.routes.aa.run_sync", new=AsyncMock(return_value=store_record)),
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                r = await client.get("/api/character/Sihtric/aas")
    ...
    async def test_census_down_no_cache_returns_503(self, app) -> None:
        ...
        with (
            patch("web.routes.aa.current_world", return_value="Varsoon"),
            patch("web.routes.aa.aa_cache.get_stale", return_value=(None, False)),
            patch("web.routes.aa.run_sync", new=AsyncMock(return_value=None)),
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                r = await client.get("/api/character/Sihtric/aas")
    ...
    async def test_char_not_found_returns_404(self, app) -> None:
        ...
        with (
            ...
            patch("web.routes.aa.aa_cache.get_stale", return_value=(None, False)),
            patch("web.routes.aa.run_sync", new=AsyncMock(return_value=None)),
            patch("web.routes.aa.shared_census_client", return_value=mock_cm),
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
    ...
    async def test_live_fetch_happy_path_stores_and_caches(self, app) -> None:
        ...
        with (
            patch("web.routes.aa.current_world", return_value="Varsoon"),
            patch("web.routes.aa.aa_cache.get_stale", return_value=(None, False)),
            patch("web.routes.aa.run_sync", side_effect=_fake_run_sync),
            patch("web.routes.aa.shared_census_client", return_value=mock_cm),
            patch("web.routes.aa.load_tree_index", return_value={1: {"type": "class", "name": "Templar"}}),
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                r = await client.get("/api/character/Sihtric/aas")
    ...
    async def test_stale_cache_hit_triggers_background_refresh(self, app) -> None:
        ...
        with (
            patch("web.routes.aa.current_world", return_value="Varsoon"),
            patch("web.routes.aa.aa_cache.get_stale", return_value=(cached, True)),
            patch("web.routes.aa.asyncio.create_task") as mock_task,
        ):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                r = await client.get("/api/character/Sihtric/aas")
...
class TestGetSpellEffects:
    async def test_no_row_found_returns_empty_effects(self, app) -> None:
        ...
        with patch("web.routes.aa.find_by_crc", return_value=None):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
    ...
    async def test_row_with_effects_returns_parsed_list(self, app) -> None:
        ...
        with patch("web.routes.aa.find_by_crc", return_value=row):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
    ...
    async def test_tier_zero_passes_none_to_find_by_crc(self, app) -> None:
        ...
        with patch("web.routes.aa.find_by_crc", side_effect=_capture):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
