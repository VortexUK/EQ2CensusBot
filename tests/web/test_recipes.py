...
async def test_search_recipes_no_filters_returns_empty(app):
    ...
    with patch("web.routes.recipes.RECIPES_DB_PATH", mock_db):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_search_recipes_class_filter_without_items_db(app):
    ...
    with (
        patch("web.routes.recipes.RECIPES_DB_PATH", mock_recipes_db),
        patch("web.routes.recipes.ITEMS_DB_PATH", mock_items_db),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
