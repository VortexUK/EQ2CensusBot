...
if valid:
    r = valid[0]
    print(f"Expected winner (highest valid): id={r['id']} level={r['level_to_use']} tier={r['tier']} tierid={r['tierid']}")
elif invalid:
    r = invalid[0]
    print(f"No valid items — fallback winner: id={r['id']} level={r['level_to_use']} tier={r['tier']} tierid={r['tierid']}")
